# Python — Dev Journey

My Python learning repository as part of a structured 
return to software development.

## 🗺️ Phase 1 Progress

| # | Topic | Status | Notes |
|---|-------|--------|-------|
| 01 | Data Types & Type Conversion | ✅ Complete  | |
| 02 | Control Flow | ⏳ Not Started | |
| 03 | Functions | ⏳ Not Started | |
| 04 | Error Handling | ⏳ Not Started | |
| 05 | File I/O | ⏳ Not Started | |
| 06 | Object-Oriented Programming | ⏳ Not Started | |
| 07 | Modules & Imports | ⏳ Not Started | |
| 08 | Type Hints | ⏳ Not Started | |
| 09 | List & Dict Comprehensions | ⏳ Not Started | |
| 10 | Decorators | ⏳ Not Started | |
| 11 | Virtual Environments | ⏳ Not Started | |
| 12 | f-strings & String Formatting | ⏳ Not Started | |
| 13 | Working with JSON | ⏳ Not Started | |
| 14 | Lambda & Built-ins | ⏳ Not Started | |

## 📁 Repository Structure

Each topic has its own folder containing:
- `notes.md` — my plain English explanation of the concept
- `practice.py` — working code examples

## 🛠️ Stack
![Python](https://img.shields.io/badge/Python-3.12-blue)
